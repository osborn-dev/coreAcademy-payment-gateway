...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-gray-900 flex items-center justify-center p-6 sm:p-4 lg:p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-screen-xl mx-auto lg:grid lg:grid-cols-10 lg:gap-6">
        {/* Form Column (60%) */}
        <motion.div
          className="lg:col-span-6 w-full max-w-[700px] mx-auto lg:mx-0 p-6 sm:p-4 lg:mt-16 mt-9 lg:p-6"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
        >
          <form
            className="bg-white p-6 sm:p-8 lg:p-8 rounded-xl shadow-lg border w-full"
            onSubmit={handleSubmit}
          >
            <h1 className="text-xl sm:text-xl lg:text-2xl font-bold text-gray-800 mb-8 sm:mb-6 text-center">
          ...
          {/* Mobile-only Buttons and Why CoreAcademy Link */}
        ...
        {/* Text and Button Column (40%, Large Screens Only) */}
        <motion.div
          className="lg:col-span-4 hidden lg:flex flex-col items-center justify-center p-6 sm:p-8 lg:p-8"
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.6 }}
        >
          <h2 className="text-xl lg:text-2xl font-bold text-white mb-4 text-center">
            Not Sure Which Dev Path to Choose?
          </h2>
          <p className="text-base text-white mb-6 text-center">
            Book a free session with our experts to find the perfect coding journey for you!
          </p>
          <motion.button
            className="p-3 bg-blue-500 text-white rounded-md hover:bg-blue-600 font-semibold shadow-md"
            whileHover={{ scale: 1.05 }}
            animate={{ y: [0, -10, 0] }}
            transition={{ repeat: Infinity, duration: 2 }}
            onClick={() => alert("Calendar modal placeholder")} // Placeholder for modal
          >
            Book a Free Session
          </motion.button>
        </motion.div>
